(*
Cracking a password from hash. The password is a four letter word
containing letters 'k' and 'l' encoded as 4 8bit vectors (a=0 ... z=25)
and then hashed according to the following algorithm.

const size_t n = 4;
vector <uint8_t > msg = {?, ?, ?, ?};
uint8_t h = msg[0];
for (size_t i = 1; i < n; i++) h = (h << 1) ^ msg[i];
cout << static_cast<size_t >(h) << endl;
*)
